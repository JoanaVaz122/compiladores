...
namespace udf {
  ...
  public:
    symbol(bool constant, int qualifier, std::shared_ptr<cdk::basic_type> type, const std::string &name, bool initialized,
           bool function, long value, bool forward = false) :
        _type(type), _name(name), _value(value), _constant(constant), _qualifier(qualifier), _initialized(initialized), _function(
