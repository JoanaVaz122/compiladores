...
static std::string qualifier_name(int qualifier) {
  ...
  if (qualifier == tFORWARD) return "import";
...
void udf::xml_writer::do_function_declaration_node(udf::function_declaration_node *const node, int lvl) {
  ...
  os() << qualifier_name(node->qualifier());
  closeTag("qualifier", lvl + 2);
  openTag("identifier", lvl + 2);
  os() << node->identifier();
  closeTag("identifier", lvl + 2);
  openTag("type", lvl + 2);
  os() << std::string(lvl + 4, ' ') << to_string(node->type()) << std::endl;
  closeTag("type", lvl + 2);
  closeTag(node, lvl);
}
...
void udf::xml_writer::do_object_node(udf::object_node *const node, int lvl) {
  //TODO
  do_unary_operation(node, lvl);
}
...
void udf::xml_writer::do_tensor_index_node(udf::tensor_index_node *const node, int lvl) {
  ...
  openTag("index", lvl + 2);
  node->indices()->accept(this, lvl + 4);
...
void udf::xml_writer::do_tensor_dim_node(udf::tensor_dim_node *const node, int lvl) {
  ...
  os() << std::string(lvl + 2, ' ') << "<dimension>" << node->tensor() << "</dimension>" << std::endl;
...
void udf::xml_writer::do_left_index_node(udf::left_index_node *const node, int lvl) {
  //TODO
  //ASSERT_SAFE_EXPRESSIONS;
